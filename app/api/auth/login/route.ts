...
export async function POST(request: NextRequest) {
  ...
  if (!credentialsConfigured()) return NextResponse.json({ error: "Local administrator credentials are not configured." }, { status: 503 });
  let candidate = "";
  ...
  if (!session) return NextResponse.json({ error: "Local session could not be created." }, { status: 503 });
